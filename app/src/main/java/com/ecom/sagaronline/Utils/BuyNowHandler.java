...
public class BuyNowHandler extends SQLiteOpenHelper {

    //private static String DB_NAME = "database13";
    private static String DBS_NAME = "database12";
    private static int DB_VERSION = 3;
    private SQLiteDatabase db;

  //  public static final String CART_TABLE = "cart";
    public static final String BUY_NOW_TABLE ="buy";
    ...
    public BuyNowHandler(Context context) {
        super(context, DBS_NAME, null, DB_VERSION);
    }
    @Override
    public void onCreate(SQLiteDatabase db) {
        this.db=db;
        String exe = "CREATE TABLE IF NOT EXISTS " + BUY_NOW_TABLE
    ...
    public boolean setCart(HashMap<String, String> map, Float Qty) {
        ...
        if (isInCart(map.get(COLUMN_CID))) {
            db.execSQL("update " + BUY_NOW_TABLE + " set " + COLUMN_QTY + " = '" + Qty + "'," + COLUMN_PRICE + " = '" + map.get(COLUMN_PRICE) + "' where " + COLUMN_CID + "=" + map.get(COLUMN_CID));
            return false;
        } else {
            ...
            db.insert(BUY_NOW_TABLE, null, values);
    ...
    public boolean isInCart(String id) {
        ...
        String qry = "Select *  from " + BUY_NOW_TABLE + " where " + COLUMN_CID + " = " + id;
    ...
    public int getCartCount() {
        db = getReadableDatabase();
        String qry = "Select *  from " + BUY_NOW_TABLE;
        Cursor cursor = db.rawQuery(qry, null);
    ...
    public String getTotalAmount() {
        db = getReadableDatabase();
        String qry = "Select SUM(" + COLUMN_PRICE + ") as total_amount  from " + BUY_NOW_TABLE;
        Cursor cursor = db.rawQuery(qry, null);
        cursor.moveToFirst();
    ...
    public String getTotalMRP() {
        db = getReadableDatabase();
        String qry = "Select SUM(" + COLUMN_MRP + ") as total_mrp  from " + BUY_NOW_TABLE;
        Cursor cursor = db.rawQuery(qry, null);
        cursor.moveToFirst();
...
//            map.put(COLUMN_DESC, cursor.getString(cursor.getColumnIndex(COLUMN_DESC)));
    ...
    public ArrayList<HashMap<String, String>> getCartProduct(int product_id) {
        ArrayList<HashMap<String, String>> list = new ArrayList<>();
        db = getReadableDatabase();
        String qry = "Select *  from " + BUY_NOW_TABLE+ " where " + COLUMN_CID + " = " + product_id;
        Cursor cursor = db.rawQuery(qry, null);
        cursor.moveToFirst();
        for (int i = 0; i < cursor.getCount(); i++) {
            HashMap<String, String> map = new HashMap<>();
...
//            map.put(COLUMN_DESC, cursor.getString(cursor.getColumnIndex(COLUMN_DESC)));
    ...
    public String getColumnRewards() {
        db = getReadableDatabase();
        String qry = "SELECT rewards FROM "+ BUY_NOW_TABLE;
        Cursor cursor = db.rawQuery(qry, null);
        cursor.moveToFirst();
    ...
    public void clearCart() {
        db = getReadableDatabase();
        db.execSQL("delete from " + BUY_NOW_TABLE);
    }
    ...
    public void removeItemFromCart(String id) {
        db = getReadableDatabase();
        db.execSQL("delete from " + BUY_NOW_TABLE + " where " + COLUMN_CID + " = " + id);
    }
